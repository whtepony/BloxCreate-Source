    ...
    <div class="container mb-25">
        <div class="grid-x grid-margin-x">
            ...
            <div class="cell small-12 medium-3 text-center">
                ...
                <a href="{{ route('users.profile', ['id' => $item->creator->id, 'username' => $item->creator->username]) }}">
                    <div class="item-creator-avatar">
                        <img class="item-creator-avatar-image" src="{{ $creatorImage }}">
                    </div>
                </a>
                <a href="{{ route('users.profile', ['id' => $item->creator->id, 'username' => $item->creator->username]) }}" class="item-creator-username">{{ $item->creator->username }}</a>
                ...
                <div class="grid-x grid-margin-x align-middle reseller">
                    <div class="cell small-5 medium-5">
                        <div class="item-private-seller-user-holder">
                            <a href="{{ route('users.profile', ['id' => $reseller->seller->id, 'username' => $reseller->seller->username]) }}">
                                <div class="item-private-seller-avatar">
                                    <img class="item-private-seller-avatar-image" src="{{ $reseller->seller->headshot() }}">
                                </div>
                            </a>
                            <a href="{{ route('users.profile', ['id' => $reseller->seller->id, 'username' => $reseller->seller->username]) }}" class="item-private-seller-username">{{ $reseller->seller->username }}</a>
                        </div>
                    </div>
                    ...
                    <div class="cell small-6 medium-2">
                        <div class="suggested-item">
                            <a href="{{ route('market.show', ['id' => $setItem->id]) }}" title="{{ $setItem->name }}">
                                <img class="market-item-thumbnail" src="{{ $setItem->thumbnail() }}">
                            </a>
            ...
            @forelse ($item->comments() as $comment)
                <div class="item-comment grid-x grid-margin-x">
                    <div class="cell small-3 medium-2 text-center">
                        <a href="{{ route('users.profile', ['id' => $comment->creator->id, 'username' => $comment->creator->username]) }}">
                            <img src="{{ $comment->creator->thumbnail() }}">
                        </a>
                        <a href="{{ route('users.profile', ['id' => $comment->creator->id, 'username' => $comment->creator->username]) }}" class="comment-creator">{{ $comment->creator->username }}</a>
                    </div>
                ...
                <div class="cell small-6 medium-3">
                    <div class="suggested-item">
                        <a href="{{ route('market.show', ['id' => $suggestion->id]) }}" title="{{ $suggestion->name }}">
                            <img class="market-item-thumbnail" src="{{ $suggestion->thumbnail() }}">
                        </a>
                        <a href="{{ route('market.show', ['id' => $suggestion->id]) }}" class="market-item-name" title="{{ $suggestion->name }}">{{ $suggestion->name }}</a>
                        <div class="market-item-creator">Creator: <a href="{{ route('users.profile', ['id' => $suggestion->creator->id, 'username' => $suggestion->creator->username]) }}">{{ $suggestion->creator->username }}</a></div>
